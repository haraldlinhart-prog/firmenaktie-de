...
export default function Home() {
  ...
  useEffect(() => {
    // dangerouslySetInnerHTML never executes <script> tags (browser spec, not a Next.js quirk),
    // so the Revive banner script has to be injected via document.createElement to actually run.
    if (!reviveRef.current || reviveRef.current.childElementCount > 0) return
    const ins = document.createElement('ins')
    ins.setAttribute('data-revive-zoneid', '6')
    ins.setAttribute('data-revive-id', '0b01ba1194fdc0e89c6321458dbc5814')
    reviveRef.current.appendChild(ins)
    const script = document.createElement('script')
    script.async = true
    script.src = '//ads.pan21.com/www/delivery/asyncjs.php'
    reviveRef.current.appendChild(script)
  }, [])

  useEffect(() => {
    // Same dangerouslySetInnerHTML issue: the <script> defining pan21NlSubmit never ran,
    // so the visible "Jetzt anmelden" button's onclick="pan21NlSubmit()" threw a silent
    // ReferenceError on click. Defining it via a real executed script fixes that.
    if ((window as any).__pan21NlInit) return
    ;(window as any).__pan21NlInit = true
...
async function pan21NlSubmit(){
  ...
  useEffect(() => {
    // Statcounter's two <script> tags have the exact same non-execution problem.
    if ((window as any).__scInit) return
    ;(window as any).__scInit = true
    ;(window as any).sc_project = 11857415
    ;(window as any).sc_invisible = 1
    ;(window as any).sc_security = 'aa23c2a9'
    const s = document.createElement('script')
...
<footer className="border-t border-champ/8 py-12 px-4 sm:px-8 bg-ink">
      ...
      {/* <!-- REVIVE:START --> */}
<div ref={reviveRef} style={{display: 'flex', justifyContent: 'center', margin: '16px 0'}} />
{/* <!-- REVIVE:END --> */}
{/* <!-- BEEHIIV:START --> */}
